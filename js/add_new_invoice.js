...
function removeRow(row_id) {
  if (rows == 1)
    alert("Không thể xóa chỉ một hàng ở đó!");
  else {
...
function checkAvailableQuantity(value, id) {
  ...
  if (Number.parseInt(xhttp.responseText) == 0) {
    ...
    document.getElementById("medicine_name_error_" + id).innerHTML = "Hết Hàng";
    return -1;
  }
  else if (value > Number.parseInt(xhttp.responseText)) {
    document.getElementById("quantity_error_" + id).style.display = "block";
    document.getElementById("quantity_error_" + id).innerHTML = "Chỉ Có " + xhttp.responseText + " trong Kho!";
    return -2;
  }
  return 999;
}
...
function addInvoice() {
  ...
  if (!notNull(customers_name.value, "customer_name_error"))
    ...
    document.getElementById("customer_name_error").innerHTML = "Khách hàng không tồn tại!";
    customers_name.focus();
  }
  else if (isInvoiceExist(invoice_number.value) == "true")
    document.getElementById("invoice_acknowledgement").innerHTML = "Đã Lưu Hóa Đơn !";
    ...
    for (var i = 1; i < row_count; i++) {
      ...
      if (!notNull(medicine_name.value, medicine_name_error.getAttribute('id')))
        ...
        medicine_name_error.innerHTML = "Thuốc Không Tồn Tại !";
        medicine_name.focus();
      }
        ...
        quantity_error.innerHTML = "Tăng Số Lượng Hoặc Loại Bỏ !";
        quantity.focus();
      }
