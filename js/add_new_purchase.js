...
function checkInvoice(invoice_number, error) {
  ...
  if (xhttp.responseText == "true") {
    ...
    document.getElementById(error).innerHTML = "Đã Thêm !";
    return true;
  }
  else
...
function addPurchase() {
  ...
  if (!notNull(suppliers_name.value, "supplier_name_error"))
    ...
    document.getElementById("supplier_name_error").innerHTML = "Nhà Cung Cấp Không Tồn Tại !";
    suppliers_name.focus();
  }
    ...
    for (var i = 1; i < row_count; i++) {
      ...
      if (!notNull(medicine_name.value, medicine_name_error.getAttribute('id')))
        ...
        quantity_error.innerHTML = "Tăng Số Lượng Hoặc Loại Bỏ Hàng !"
        quantity.focus();
      }
