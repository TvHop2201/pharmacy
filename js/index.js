...
function validateAndSetup() {
  ...
  if (!validateName(pharmacy_name.value, 'pharmacy_name_error'))
    ...
    document.getElementById('username_error').innerHTML = "không được chứa khoảng trắng!";
    username.focus();
  }
  else if (password.value.indexOf(' ') >= 0) {
    document.getElementById('password_error').style.display = "block";
    document.getElementById('password_error').innerHTML = "không được chứa khoảng trắng!";
    password.focus();
  }
  else if (password.value.length < 6) {
    document.getElementById('password_error').style.display = "block";
    document.getElementById('password_error').innerHTML = "Phải Có Độ Dài Từ 6 Ký Tự Trở Lên !";
    password.focus();
  }
  else if (password.value != confirm_password.value) {
    document.getElementById('password_error').style.display = "none";
    document.getElementById('confirm_password_error').style.display = "block";
    document.getElementById('confirm_password_error').innerHTML = "Password Không Trùng Khớp !";
    confirm_password.focus();
  }
  else {
    document.getElementById('confirm_password_error').style.display = "none";
    var confirmation = prompt("Vui Lòng Xác Nhận !");
...
function updateUsernamePassword() {
  ...
  if (!notNull(username.value, 'username_error'))
    ...
    xhttp.send();
    if (xhttp.responseText == "true") {
      alert("Tên người dùng và mật khẩu mới đã được đặt thành công ..");
      displayLoginForm();
    }
    else
      alert("Thất Bại !!!");
